...
public class DriveBaseTele extends Command {
  ...
  private void driveControls(){
    ...
    // lStickY = -RobotUtil.capper(-x + y,2);
    // rStickY = -RobotUtil.capper(x + y,2);
    // // if(Robot.decelerator.getSelected().equals(true)){
    // //   lHandY *= 0.8;
    // //   rHandY *= 0.8;
    // //   decelerate();
    // // }
    // Robot.driveBase.percentOutput(lStickY, rHandY);

    double triggerVal = Robot.oi.driverRawAxis(RobotMap.RIGHT_TRIGGER) - Robot.oi.driverRawAxis(RobotMap.LEFT_TRIGGER);
    double stick = RobotUtil.scale(Robot.oi.driverRawAxis(RobotMap.LEFT_STICK_X), RobotMap.TURNING_RATE);

    Robot.driveBase.percentOutput(triggerVal + stick, triggerVal - stick);
  }
  ...
  protected void interrupted() {
    Robot.driveBase.percentOutput(0,0);
  }
}
